...
- **`tablet/`** — Flutter (Dart) Android kiosk app for resident-facing tablets. Talks to the same NestJS backend over REST + WebSocket. Runs in three flavors: `dev` (cloud dev API), `local` (local backend testing), and `prod`. Local dev: `flutter run --flavor dev -t lib/main.dart`; local backend testing: `flutter run --flavor local -t lib/main.dart`.
